...
import { readJourneySnapshot } from "./journeyWorkspace.js";
...
function bodyFor(root: string, artifactId: string, artifactPath: string, workflowStatus?: string, receipt?: string): string {
  let path: string;
  try { path = safePath(root, artifactPath); }
  catch (error) { return `<p class="warning">Cannot open artifact: ${escapeHtml(error instanceof Error ? error.message : String(error))}</p>`; }
...
export function openJourneyArtifactPanel(root: string, artifactId: string, artifactPath: string, workflowStatus?: string, receipt?: string): vscode.WebviewPanel {
  ...
  const refreshMetadata = (): void => {
    try {
      const artifact = readJourneySnapshot(root).artifacts.find((item) => item.id === artifactId || item.path === artifactPath);
      liveStatus = artifact?.status ?? workflowStatus;
      liveReceipt = artifact?.receipt ?? receipt;
    } catch { /* retain last good metadata while the workspace is being updated */ }
  };
  refreshMetadata();
  ...
  const update = (): void => { refreshMetadata(); void panel.webview.postMessage({ type: "artifact-update", body: bodyFor(root, artifactId, artifactPath, liveStatus, liveReceipt) }); };
  ...
  const receiptWatcher = vscode.workspace.createFileSystemWatcher(new vscode.RelativePattern(root, ".sdlc/context-receipts/**"));
  panel.onDidDispose(() => { workflowWatcher.dispose(); receiptWatcher.dispose(); });
  workflowWatcher.onDidCreate(update); workflowWatcher.onDidChange(update); workflowWatcher.onDidDelete(update);
  receiptWatcher.onDidCreate(update); receiptWatcher.onDidChange(update); receiptWatcher.onDidDelete(update);
  return panel;
}
