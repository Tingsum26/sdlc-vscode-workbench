...
class JourneyItem extends vscode.TreeItem {
  ...
  constructor(label: string, description: string, tooltip: string, artifactPath?: string, artifactStatus?: string, artifactReceipt?: string) {
    super(label); this.description = description; this.tooltip = tooltip; this.artifactPath = artifactPath;
    this.artifactStatus = artifactStatus; this.artifactReceipt = artifactReceipt;
...
class JourneyProvider implements vscode.TreeDataProvider<JourneyItem> {
  ...
  getChildren(): JourneyItem[] {
    ...
    return [...base, ...s.artifacts.map((artifact) => new JourneyItem(artifact.id, `${artifact.status} · receipt ${artifact.receipt}`, `${artifact.path}\n${artifact.receipt}\nClick to open the HTML report`, artifact.path, artifact.status, artifact.receipt)), new JourneyItem("Repositories", s.affectedRepositories.join(", ") || "none", "Linked code repositories")];
  }
}
