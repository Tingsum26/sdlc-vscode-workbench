...
class JourneyProvider implements vscode.TreeDataProvider<JourneyItem> {
  ...
  getChildren(): JourneyItem[] {
    ...
    const base = [new JourneyItem(`Journey · ${s.journeyId}`, `${s.status} · ${s.currentStage}`, `Workflow ${s.workflowId}\nBranch ${s.branch}`), new JourneyItem("Current stage", s.nextRole ? `${s.currentStage} → ${s.nextRole}` : s.currentStage, "Declared by .sdlc/workflow.json")];
    if (this.section === "sdlc.myWork" || this.section === "sdlc.scrumMaster") return [...base, new JourneyItem("Next action", "Run Journey Coordinator Agent", `Copilot: /resume-workflow ${s.workflowId}`), new JourneyItem("Tickets", s.sourceTickets.join(", ") || "none", "Source tickets in workflow.json")];
