...
import { openJourneyArtifactPanel } from "./markdownArtifactPanel.js";
...
class JourneyItem extends vscode.TreeItem {
  ...
  constructor(label: string, description: string, tooltip: string, artifactPath?: string) {
    super(label); this.description = description; this.tooltip = tooltip; this.artifactPath = artifactPath;
    if (artifactPath) { this.command = { command: "sdlc.openJourneyArtifact", title: `Open ${label} report`, arguments: [this] }; this.contextValue = "sdlcJourneyArtifact"; }
...
class JourneyProvider implements vscode.TreeDataProvider<JourneyItem> {
  ...
  getChildren(): JourneyItem[] {
    ...
    if (this.section === "sdlc.myWork" || this.section === "sdlc.scrumMaster") {
      ...
      return [...base, new JourneyItem("Next Agent", s.nextAgent ?? "none", "由 stageOrder 和当前输出状态确定"), new JourneyItem("Current Agent Report", `${s.currentOutputStatus} · open HTML`, s.currentOutputPath ?? "No current output", s.currentOutputPath), new JourneyItem("Next action", next, `Copilot: /resume-workflow ${s.workflowId}`), new JourneyItem("Tickets", s.sourceTickets.join(", ") || "none", "Source tickets in workflow.json")];
    }
    ...
    return [...base, ...s.artifacts.map((artifact) => new JourneyItem(artifact.id, `${artifact.status} · receipt ${artifact.receipt}`, `${artifact.path}\n${artifact.receipt}\nClick to open the HTML report`, artifact.path)), new JourneyItem("Repositories", s.affectedRepositories.join(", ") || "none", "Linked code repositories")];
  }
}
